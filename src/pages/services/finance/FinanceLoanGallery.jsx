...
function FinanceLoanGallery() {
  ...
  const loanTypePillItems = useMemo(() => {
    return typeStrip.map((ct) => ({
      id: ct.id,
      icon: ct.icon,
      label: ct.label,
      count: ct.id === 'All' ? loans.length : loans.filter((l) => l.type === ct.id).length,
    }));
  }, [typeStrip, loans]);
  ...
  return (
    <PageContainer>
      ...
      <PageHeader
        ...
        subtitle="Home loans, vehicle loans, and more — pre-approved, competitive rates, no hidden charges."
      />
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <EmptyStateCard icon="fa-building-columns" title="No loan products found" message="Try selecting another type or check back later." />
        )}
      </div>
