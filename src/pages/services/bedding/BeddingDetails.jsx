...
export default function BeddingDetails() {
  ...
  // Reset selected variant whenever product changes
  useEffect(() => {
  ...
  const displayedItem = useMemo(() => {
    ...
    const overviewHighlights = [
      {
        label: isMultiVariant ? 'Selected Size' : 'Standard Dimensions',
        value: currentVariant.name,
        icon: 'fa-bed',
      ...
      ...(rawItem.overviewHighlights || []).slice(0, 2),
    ];
    ...
    return {
      ...
      priceNote: isMultiVariant
        ? `Selected Size: ${currentVariant.name} · Direct Manufacturer Warranty · Free Delivery`
        : rawItem.priceNote,
      keyAttributes,
      overviewHighlights,
    };
  }, [rawItem, currentVariant, variants.length]);

  const similarBedding = useMemo(() => {
    if (!rawItem) return [];
    return dummyBedding
      .filter((b) => String(b.id) !== String(rawItem.id))
      .slice(0, 6);
  }, [rawItem]);
    ...
    <section className="rounded-3xl bg-white border border-gray-200/70 p-6 sm:p-8 shadow-xs space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Official Peps India Pricing & Available Size Lineup
            </p>
          </div>
        </div>
      ...
      <div className="space-y-3 pt-2">
        <h4 className="text-sm font-bold text-brand-charcoal flex items-center gap-2">
          ...
          Compare All {variants.length} Sizes for {rawItem.title}
        </h4>
